...
export default async function TrackingPage({ searchParams }: Props) {
  ...
  return <main><p className="eyebrow">{snapshot.project.provider} · {snapshot.project.slug}</p><h1>{snapshot.manifest.project.name}</h1><p className="lede">Branch <strong>{snapshot.branch.name}</strong> at commit <code>{snapshot.branch.commit.slice(0, 12)}</code>.</p><nav className="actions">{branches.map((item) => <Link key={item.name} className={item.name === snapshot.branch.name ? "button" : "button secondary"} href={`/tracking?project=${encodeURIComponent(project.slug)}&branch=${encodeURIComponent(item.name)}`}>{item.name}</Link>)}</nav><section><h2>Tracking sources</h2><ul>{Object.entries(snapshot.sources).map(([name, source]) => <li key={name}><strong>{name}</strong>: {((parseRecordCount(source)))} records</li>)}</ul></section><section><h2>Project catalog</h2><p>{workspace.catalog.services.length} services · {workspace.catalog.components.length} components · {workspace.catalog.areas.length} areas</p></section><section><h2>New planned improvement</h2><NewPlannedImprovementForm project={project.slug} branch={snapshot.branch.name} expectedBranchCommit={snapshot.branch.commit} catalog={workspace.catalog} relationships={workspace.relationships} /></section><section><h2>New development log</h2><NewDevelopmentLogForm project={project.slug} branch={snapshot.branch.name} expectedBranchCommit={snapshot.branch.commit} /></section><section><h2>Planned improvements</h2><p>Each record opens in a validated edit form.</p><ul>{planned.records.map((record) => <li key={record.id}><EditPlannedImprovementForm project={project.slug} branch={snapshot.branch.name} expectedBranchCommit={snapshot.branch.commit} record={record} catalog={workspace.catalog} relationships={workspace.relationships} /></li>)}</ul></section><Link href="/">Return to ORBIT</Link></main>;
}
