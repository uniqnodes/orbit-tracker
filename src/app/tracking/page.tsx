...
import { plannedImprovementDocumentSchema, proposedImprovementDocumentSchema } from "@/lib/tracking/schema";
...
export default async function TrackingPage({ searchParams }: { searchParams: Promise<{ project?: string; branch?: string }> }) {
  ...
  return <main>
    ...
    <section><h2>New proposal</h2><NewProposalForm {...context} /></section>
    <section><h2>Proposals</h2><ul>{proposals.records.map((record) => <li key={record.id}><EditProposalForm {...context} record={record} /></li>)}</ul></section>
